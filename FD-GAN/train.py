...
#name = market1501, opt.dataroo = data_dir, height:256, width : 128, batch_size : 16, workers : 4, pose_aug :no
def get_data(name, data_dir, height, width, batch_size, workers, pose_aug):
    ...
    # use combined trainval set for training as default 使用组合trainval培训设置为默认
...
def main():
    opt = Options().parse()
    #opt.dataset = market1501, opt.dataroot, opt.height:256, opt.width : 128, opt.batch_size : , opt.workers : 4, opt.pose_aug
    ...
    print('dataset.trainval')
    print(len(dataset.trainval))
    ...
    for epoch in range(1, opt.niter + opt.niter_decay + 1):
        ...
        # 数据迭代读取的循环函数，
        # 是先调用DataLoader类的__iter__方法，该方法里面再调用DataLoaderIter类的初始化操作__init__
